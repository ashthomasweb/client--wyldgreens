...
// Dev note: Issues with current architecture led to NodeMailer not functioning correctly in any attempted module setups.
// Leave nodemailer functions and templates in post route if hosted with A2 hosting. Small server file allows for this 
// to not be a performance issue at current scale. Possible that on a newly built server even at A2 module exporting could work.
// App duplication occurs when a non-SSL connection is attempted, potentially due to the current setup where wyldgreens is a subdomain 
// of the parent. 

// Ideally site should be re-architected on a host that allows external smtp, authorization performed with OAuth.
// Current registrar GoDaddy creates problems with most hosting companies that are node friendly and affordable.
// Potential solution that requires DNS migration and full refactor of nodemailer authorization is heroku, DNSimple, 
// and Google OAuth with a Google for Business email. This service architecture will not work with any current 
...
app.post("/", function (req, res) {
  ...
  let finalConfirm = JSON.parse(
    mailConfirmation(user_name, user_email, message)
  );
